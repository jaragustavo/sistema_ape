<?php
class Concursos extends Conectar
{
    ...
    public function get_tramites_concursos($tipo_solicitud)
    {
        $conectar = parent::Conexion();
        $sql = "SELECT id as tramite_id, nombre as tramite, 
        url from tramites where tipo_tramite_id = 1 and activo =true
    ...
    public function get_secciones($tipo_solicitud, $tramite_code, $usuario_id)
    {
        ...
        if ($tipo_solicitud == 'CERT') {
            ...
            $certificacion = 'false';
        }  elseif ($tipo_solicitud == 'CONCURSO') {
            $certificacion = 'false';
        }
        $conectar = parent::Conexion();
     
        $sql = "SELECT 'V Concurso Investigación: ' as seccion_nombre, 
        ...
        try {
            ...
            if (is_array($tiposDocumentos)) {
                ...
                while ($item < count($tiposDocumentos)) {
                    ...
                    $nombre_archivo = Concursos::buscarNombreArchivo($idTypeFile, $tramite_id, $cedula, $db);
    ...
    public function delete_tramite_gestionado($id_tramite_gestionado, $usuario_id, $fecha_hora, $estado_tramite_id)
    {
        ...
        try {
            ...
            // actualización tramites_gestionados
            $sql = "UPDATE public.tramites_gestionados
                    SET activo = false, estado_tramite_id = :estado_tramite_id
                    WHERE id = :id_tramite_gestionado;";
            $stmt = $db->prepare($sql);
            $params = [
                ':estado_tramite_id' => $estado_tramite_id,
                ':id_tramite_gestionado' => $id_tramite_gestionado
            ];
            // Construir la consulta SQL completa con los valores
            $sql_with_values = str_replace(
                [':estado_tramite_id', ':id_tramite_gestionado'],
                [$params[':estado_tramite_id'], $params[':id_tramite_gestionado']],
                $sql
            );
           ...
           // Inserción de movimiento
            ...
            $sql_with_values = str_replace(
            array_keys($params),
            array_map(function($value) use ($db) { return $db->quote($value); }, array_values($params)),
            $sql
            );
            ...
            $docs_existentes = $stmt->fetchAll(PDO::FETCH_ASSOC);
